...
const ExpertProfile = () => {
  ...
  const fetchList = async (type) => {
    try {
      const response = await axios.get(`/user/${userId}/${type}`);
      setListType(type);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-2">
          <div className="bg-white p-2 sm:p-6 rounded-lg shadow-lg w-full max-w-xs sm:max-w-lg">
            ...
            <div className="w-full h-[60vh] min-h-[300px]">
              {isMobile() ? (
                <iframe
                  src={`https://docs.google.com/gview?embedded=true&url=${user?.expertProfile?.expertiseCertificate}`}
